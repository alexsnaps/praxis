...
fn stream_buffer_pipeline_extracts_and_routes() {
    let gpt4_port = start_backend("gpt4-response");
    let default_port = start_backend("default-response");
    let proxy_port = free_port();
    let config = Config::from_yaml(&stream_buffer_routing_yaml(proxy_port, gpt4_port, default_port)).unwrap();
    let addr = crate::common::start_proxy(&config);

    let raw = http_send(
        &addr,
        &json_post("/v1/chat", r#"{"model":"gpt-4","user_id":"u-1","prompt":"hi"}"#),
    );
    assert_eq!(parse_status(&raw), 200, "stream-buffer gpt-4 routing should return 200");
    assert_eq!(
        parse_body(&raw),
        "gpt4-response",
...
fn multi_listener_per_listener_filter_chains() {
    let echo_port = start_echo_backend();
    let gpt4_port = start_backend("gpt4-routed");
...
"#
    ...
    let raw = http_send(
        ...
        &json_post("/v1/chat", r#"{"model":"gpt-4","user_id":"u-99"}"#),
    );
    assert_eq!(parse_status(&raw), 200, "extraction listener gpt-4 should return 200");
    assert_eq!(
        parse_body(&raw),
        "gpt4-routed",
        "extraction listener should route gpt-4 to gpt4 cluster"
    );
    ...
    let payload = r#"{"model":"gpt-4","prompt":"hi"}"#;
...
fn stream_buffer_routing_yaml(proxy_port: u16, gpt4_port: u16, default_port: u16) -> String {
    format!(
        r#"
listeners:
  - name: default

...
fn multi_field_extracts_both_fields() {
    ...
    let raw = http_send(&addr, &json_post("/v1/chat", r#"{"model":"gpt-4","user_id":"u-42"}"#));
    assert_eq!(parse_status(&raw), 200, "multi-field extraction should return 200");
    let body = parse_body(&raw);
    let body_lower = body.to_lowercase();
    assert!(
        body_lower.contains("x-model: gpt-4"),
...
fn multi_field_missing_one_still_extracts_other() {
    ...
    let raw = http_send(&addr, &json_post("/v1/chat", r#"{"model":"gpt-4"}"#));
...
fn multi_field_routes_by_extracted_model() {
    let gpt4_port = start_backend("gpt4-backend");
    let default_port = start_backend("default-backend");
    let proxy_port = free_port();

    let yaml = multi_field_routing_yaml(proxy_port, gpt4_port, default_port);
    let config = Config::from_yaml(&yaml).unwrap();
    let addr = start_proxy(&config);

    let raw = http_send(&addr, &json_post("/v1/chat", r#"{"model":"gpt-4","user_id":"u-42"}"#));
    assert_eq!(parse_status(&raw), 200, "gpt-4 model routing should return 200");
    assert_eq!(
        parse_body(&raw),
        "gpt4-backend",
        "model=gpt-4 should route to gpt4 cluster"
    );
...
fn conditional_extraction_fires_on_matching_path() {
    ...
    let raw = http_send(
        &addr,
        &json_post("/v1/chat/completions", r#"{"model":"gpt-4","prompt":"hi"}"#),
    );
    assert_eq!(parse_status(&raw), 200, "matching path extraction should return 200");
    let body = parse_body(&raw);
    assert!(
        body.to_lowercase().contains("x-model: gpt-4"),
...
fn conditional_extraction_skips_on_non_matching_path() {
    ...
    let raw = http_send(
        &addr,
        &json_post("/other/endpoint", r#"{"model":"gpt-4","prompt":"hi"}"#),
    );
...
fn body_limit_allows_small_body_with_extraction() {
    ...
    let raw = http_send(&addr, &json_post("/v1/chat", r#"{"model":"gpt-4","prompt":"hello"}"#));
...
fn body_limit_rejects_oversized_body() {
    ...
    let large_body = format!(r#"{{"model":"gpt-4","prompt":"{}"}}"#, "x".repeat(100));
...
fn multi_field_routing_yaml(proxy_port: u16, gpt4_port: u16, default_port: u16) -> String {
    format!(
        r#"
listeners:
  - name: default

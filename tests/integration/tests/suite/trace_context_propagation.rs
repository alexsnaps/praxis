...
#[test]
fn conditional_trace_context_only_correlates_matching_requests() {
    let trace_context = r#"      - filter: trace_context
        conditions:
          - when:
              path_prefix: "/traced""#;
    let (proxy, _upstream, _callout) = start_pre_read_proxy_with(trace_context);

    let traced = http_send(
        proxy.addr(),
        "POST /traced HTTP/1.1\r\nHost: localhost\r\nContent-Length: 4\r\nConnection: close\r\n\r\nbody",
    );
    assert_eq!(parse_status(&traced), 200);
    let traced_body = parse_body(&traced);
    assert!(
        header_from_echo(&traced_body, "traceparent").is_some(),
        "matching request must receive a forwarding traceparent:\n{traced_body}"
    );
    assert!(
        header_from_echo(&traced_body, "x-callout-traceparent").is_some(),
        "matching request must correlate its pre-read callout:\n{traced_body}"
    );

    let skipped = http_send(
        proxy.addr(),
        "POST /plain HTTP/1.1\r\nHost: localhost\r\nContent-Length: 4\r\nConnection: close\r\n\r\nbody",
    );
    assert_eq!(parse_status(&skipped), 200);
    let skipped_body = parse_body(&skipped);
    assert!(
        header_from_echo(&skipped_body, "traceparent").is_none(),
        "skipped trace_context must not add a forwarding traceparent:\n{skipped_body}"
    );
    assert!(
        header_from_echo(&skipped_body, "x-callout-traceparent").is_none(),
        "skipped trace_context must not correlate the pre-read callout:\n{skipped_body}"
    );
}

#[test]
fn body_promoted_header_activates_trace_context_before_later_callout() {
    let trace_context = r#"      - filter: trace_context
        conditions:
          - when:
              headers:
                x-trace-enabled: "on""#;
    let extra_filters = "      - filter: promote_trace_gate";
    let (proxy, _upstream, _callout) = start_pre_read_proxy_with_filters(trace_context, extra_filters);
    ...
    let body = parse_body(&raw);
    assert!(
        header_from_echo(&body, "x-callout-traceparent").is_some(),
        "a body-promoted condition must correlate the later pre-read callout:\n{body}"
    );
}
...
struct PromoteTraceGateFilter;

#[async_trait::async_trait]
impl HttpFilter for PromoteTraceGateFilter {
    ...
    async fn on_request_body(
        ...
        if end_of_stream {
            ctx.extra_request_headers
                .push((std::borrow::Cow::Borrowed("x-trace-enabled"), "on".to_owned()));
        }
        Ok(FilterAction::Continue)
    }
}
...
fn start_pre_read_proxy() -> (
    ...
    start_pre_read_proxy_with("      - filter: trace_context")
}

fn start_pre_read_proxy_with(
    trace_context: &str,
) -> (
    praxis_test_utils::ProxyGuard,
    praxis_test_utils::BackendGuard,
    ...
    start_pre_read_proxy_with_filters(trace_context, "")
}

fn start_pre_read_proxy_with_filters(
    trace_context: &str,
    extra_filters: &str,
) -> (
    praxis_test_utils::ProxyGuard,
...
"#,
    ...
    registry
        .register(
            "promote_trace_gate",
            FilterFactory::Http(Arc::new(|_| Ok(Box::new(PromoteTraceGateFilter)))),
        )
        .expect("duplicate filter name");

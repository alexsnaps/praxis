...
#[test]
fn wildcard_legacy_routes_config() {
    let wildcard_port = start_backend("wildcard-legacy");
    let default_port = start_backend("default-legacy");
    let proxy_port = free_port();
...
"#
    ...
    assert_eq!(status, 200, "wildcard in legacy routes should match");
    assert_eq!(
        body, "wildcard-legacy",
        "wildcard in legacy routes should route to wildcard cluster"
    );

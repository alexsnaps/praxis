...
///
/// Removes headers that filters deleted, then inserts or overwrites the remaining
/// headers so the Pingora response exactly mirrors the filter [`Response`].
///
/// ```ignore
/// // Requires `pingora_http::ResponseHeader` from Pingora internals.
/// use praxis_protocol::http::pingora::convert::sync_response_to_pingora;
///
/// let filter_resp = praxis_filter::Response {
...
pub(crate) fn sync_response_to_pingora(filter_resp: &Response, pingora_resp: &mut pingora_http::ResponseHeader) {
    ...
    let stale: Vec<http::header::HeaderName> = pingora_resp
        .headers
        .keys()
    ...
    for name in &stale {
        let _ = pingora_resp.remove_header(name.as_str());
    }
    ...
    for (name, value) in &filter_resp.headers {
        ...
        let _ = pingora_resp.insert_header(name.clone(), value.clone());
    }
}
...
///
/// let rejection = praxis_filter::Rejection::new(403);
...
mod tests {
    ...
    fn sync_response_copies_headers() {
        ...
        assert_eq!(
            pingora_resp.headers.get("x-response-id").unwrap(),
            "abc123",
            "synced header value should match filter response"
        );
    }
    ...
    fn sync_response_multiple_headers() {
        ...
        assert_eq!(
            pingora_resp.headers.get("x-a").unwrap(),
            "1",
            "x-a header should be synced"
        );
        assert_eq!(
            pingora_resp.headers.get("x-b").unwrap(),
            "2",
            "x-b header should be synced"
        );
    }
    ...
    fn sync_response_removes_deleted_headers() {
        ...
        assert_eq!(pingora_resp.headers.get("x-a").unwrap(), "1", "x-a should be preserved");
        assert!(
            pingora_resp.headers.get("x-b").is_none(),
            "deleted header must not survive sync"
        );
        assert_eq!(pingora_resp.headers.get("x-c").unwrap(), "3", "x-c should be preserved");
    }
    ...
    fn sync_response_preserves_status_code() {
        ...
        assert_eq!(
            ...
            "status code should be preserved after sync"
        );
    }
    ...
    fn sync_response_adds_new_headers() {
        ...
        assert_eq!(
            pingora_resp.headers.get("x-new-header").unwrap(),
            "added-by-filter",
            "new header should be added to pingora response"
        );
    }
    ...
    fn sync_response_overwrites_modified_headers() {
        ...
        assert_eq!(
            pingora_resp.headers.get("server").unwrap(),
            "praxis",
            "modified header should be overwritten"
        );
    }

...
fn validate_host_header(session: &mut Session) -> Option<Rejection> {
    ...
    let host_values: Vec<Vec<u8>> = session
        .req_header()
        .headers
    ...
    if host_values.len() > 1 {
        ...
        if host_values.iter().skip(1).all(|v| v == first) {
            ...
            if let Ok(val) = http::header::HeaderValue::from_bytes(&canonical) {
                let _ = session.req_header_mut().insert_header(http::header::HOST, val);
            }
        } else {
        ...
        match pre_read_body(pipeline, session, ctx, &request).await {
            Ok(extra_headers) => {
                ...
                for (name, value) in extra_headers {
                    let _ = session.req_header_mut().insert_header(name.clone(), value.clone());

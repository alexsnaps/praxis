...
/// [RFC 9110] hop-by-hop headers that must not be forwarded on responses.
///
/// This differs from the request list:
/// - Includes `Proxy-Authenticate` (hop-by-hop per [RFC 9110]).
...
pub(crate) fn strip_hop_by_hop_response(resp: &mut ResponseHeader) {
    if resp.headers.get("connection").is_none() {

...
pub(super) fn execute(ctx: &mut RequestCtx) -> Result<Box<HttpPeer>> {
    tracing::trace!("selecting upstream: first call takes from ctx, retry restores saved copy");
...
fn build_peer(upstream: Upstream) -> Result<Box<HttpPeer>> {
    ...
    let mut peer = HttpPeer::new(addr, upstream.tls, upstream.sni.unwrap_or_default());
...
mod tests {
    ...
    fn execute_first_call_produces_peer_and_saves_retry() {
        ...
        assert_eq!(
            &*ctx.upstream_for_retry.unwrap().address,
            "127.0.0.1:8080",
